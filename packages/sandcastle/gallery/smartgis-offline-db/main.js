...
showStatus("<b>离线数据(IndexedDB)</b><br>核心概念: tile caching with IndexedDB<br>核心 API: Resource intercept + IDB");

Sandcastle.addDefaultToolbarButton("演示 离线数据(IndexedDB)", function () {
  viewer.entities.removeAll();
  activeEntities = [];
  showStatus("<b>离线数据(IndexedDB) - 运行中</b><br>核心: tile caching with IndexedDB");
  ...
  const e = viewer.entities.add({
    ...
    label: { text: "离线数据(IndexedDB)", font: "14px sans-serif", showBackground: true, disableDepthTestDistance: Number.POSITIVE_INFINITY },
  });
...
Sandcastle.addToolbarButton("重置", function () {
  ...
  showStatus("<b>离线数据(IndexedDB)</b><br>已重置");
});
